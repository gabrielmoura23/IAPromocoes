...
namespace IAPromocoes.Infra.CrossCutting.Identity.Adm.Configuration
{
    public static class ApplicationCookieIdentityValidator
    {
        ...
        public static Func<CookieValidateIdentityContext, Task> OnValidateIdentity(TimeSpan validateInterval, Func<UsuarioAdmManager, UsuarioAdm, Task<ClaimsIdentity>> regenerateIdentity)
        {
            return async context =>
            {
                ...
                if (expired)
                {
                    var userManager = context.OwinContext.GetUserManager<UsuarioAdmManager>();
